...
public class BatteryLevelChanged extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        repository.getAllmacros().observeForever(new Observer<List<MacroStorage>>() {
            ...
            public void onChanged(List<MacroStorage> macroStorages) {
                temp = macroStorages;
                if (batterylevel != intentt.getIntExtra("level", 0)) {
                    batterylevel = intentt.getIntExtra("level", 0);
                    for (int i = 0; i < macroStorages.size(); i++) {
                        if (macroStorages.get(i).getEnabled()) {
                            for (Integer integer : macroStorages.get(i).getTriggerbattery()) {
                                if (batterylevel.equals(integer)) {
                                    try {
                                        constraintcheck(i);
                                        break;
    ...
    void constraintcheck(final int index) throws Settings.SettingNotFoundException {
        ...
        if (batteryTempTemplates != null) {
            ...
            int temperature = batterytempcheck.batterytemp(intentt);
